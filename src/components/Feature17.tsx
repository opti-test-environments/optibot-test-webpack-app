...
interface Feature17Props { data: Array<{ id: number, value: string, ts: Date }> }

function process17(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature17({ data }: Feature17Props) {
  const [result, setResult] = useState<ReturnType<typeof process17>>([])
  useEffect(() => { setResult(process17(data)) }, [data])

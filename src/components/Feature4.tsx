...
interface Feature4Props { data: Array<{ id: number, value: string, ts: Date }> }

function process4(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature4({ data }: Feature4Props) {
  const [result, setResult] = useState<ReturnType<typeof process4>>([])
  useEffect(() => { setResult(process4(data)) }, [data])

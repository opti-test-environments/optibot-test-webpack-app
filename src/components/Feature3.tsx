...
interface Feature3Props { data: Array<{ id: number, value: string, ts: Date }> }

function process3(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature3({ data }: Feature3Props) {
  const [result, setResult] = useState<ReturnType<typeof process3>>([])
  useEffect(() => { setResult(process3(data)) }, [data])

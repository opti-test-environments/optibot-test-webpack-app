...
interface Feature10Props { data: Array<{ id: number, value: string, ts: Date }> }

function process10(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature10({ data }: Feature10Props) {
  const [result, setResult] = useState<ReturnType<typeof process10>>([])
  useEffect(() => { setResult(process10(data)) }, [data])

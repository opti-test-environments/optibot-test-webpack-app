...
interface Feature2Props { data: Array<{ id: number, value: string, ts: Date }> }

function process2(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature2({ data }: Feature2Props) {
  const [result, setResult] = useState<ReturnType<typeof process2>>([])
  useEffect(() => { setResult(process2(data)) }, [data])

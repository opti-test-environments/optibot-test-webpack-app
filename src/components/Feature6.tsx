...
interface Feature6Props { data: Array<{ id: number, value: string, ts: Date }> }

function process6(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature6({ data }: Feature6Props) {
  const [result, setResult] = useState<ReturnType<typeof process6>>([])
  useEffect(() => { setResult(process6(data)) }, [data])

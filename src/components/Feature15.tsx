...
interface Feature15Props { data: Array<{ id: number, value: string, ts: Date }> }

function process15(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature15({ data }: Feature15Props) {
  const [result, setResult] = useState<ReturnType<typeof process15>>([])
  useEffect(() => { setResult(process15(data)) }, [data])

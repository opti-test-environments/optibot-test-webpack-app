...
interface Feature16Props { data: Array<{ id: number, value: string, ts: Date }> }

function process16(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature16({ data }: Feature16Props) {
  const [result, setResult] = useState<ReturnType<typeof process16>>([])
  useEffect(() => { setResult(process16(data)) }, [data])

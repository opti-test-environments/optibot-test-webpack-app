...
interface Feature12Props { data: Array<{ id: number, value: string, ts: Date }> }

function process12(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature12({ data }: Feature12Props) {
  const [result, setResult] = useState<ReturnType<typeof process12>>([])
  useEffect(() => { setResult(process12(data)) }, [data])

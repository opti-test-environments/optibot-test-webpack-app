...
interface Feature19Props { data: Array<{ id: number, value: string, ts: Date }> }

function process19(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature19({ data }: Feature19Props) {
  const [result, setResult] = useState<ReturnType<typeof process19>>([])
  useEffect(() => { setResult(process19(data)) }, [data])

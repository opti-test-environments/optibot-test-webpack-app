...
interface Feature18Props { data: Array<{ id: number, value: string, ts: Date }> }

function process18(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature18({ data }: Feature18Props) {
  const [result, setResult] = useState<ReturnType<typeof process18>>([])
  useEffect(() => { setResult(process18(data)) }, [data])

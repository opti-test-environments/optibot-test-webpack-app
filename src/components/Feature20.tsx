...
interface Feature20Props { data: Array<{ id: number, value: string, ts: Date }> }

function process20(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature20({ data }: Feature20Props) {
  const [result, setResult] = useState<ReturnType<typeof process20>>([])
  useEffect(() => { setResult(process20(data)) }, [data])

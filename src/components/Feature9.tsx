...
interface Feature9Props { data: Array<{ id: number, value: string, ts: Date }> }

function process9(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature9({ data }: Feature9Props) {
  const [result, setResult] = useState<ReturnType<typeof process9>>([])
  useEffect(() => { setResult(process9(data)) }, [data])

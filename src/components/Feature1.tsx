...
interface Feature1Props { data: Array<{ id: number, value: string, ts: Date }> }

function process1(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature1({ data }: Feature1Props) {
  const [result, setResult] = useState<ReturnType<typeof process1>>([])
  useEffect(() => { setResult(process1(data)) }, [data])

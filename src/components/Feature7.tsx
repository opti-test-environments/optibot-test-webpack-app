...
interface Feature7Props { data: Array<{ id: number, value: string, ts: Date }> }

function process7(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature7({ data }: Feature7Props) {
  const [result, setResult] = useState<ReturnType<typeof process7>>([])
  useEffect(() => { setResult(process7(data)) }, [data])

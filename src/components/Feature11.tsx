...
interface Feature11Props { data: Array<{ id: number, value: string, ts: Date }> }

function process11(items: Array<{ id: number, value: string, ts: Date }>) {
...
export function Feature11({ data }: Feature11Props) {
  const [result, setResult] = useState<ReturnType<typeof process11>>([])
  useEffect(() => { setResult(process11(data)) }, [data])
